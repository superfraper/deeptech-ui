...
const PartE = props => {
  ...
  useEffect(() => {
    (async () => {
      setIsLoading(true);
      try {
        const data = await api.getSectionFields(8, 'ART'); // Specify "ART" token type
  ...
  const sectionConfig = {
    sectionNumber: 8,
    sectionTitle: ' Part E: Information on the underlying technology',
    sectionDescription:
      'This section should be completed by the offeror or person seeking admission.',
    nextRoute: '/art/partF',
  };
        ...
        <HighlightedField key={id} fieldId={id}>
          <div className='mb-10'>
            ...
            >
              ...
              {field.field_name}
              <FlagIcon fieldId={id} section='partE' sectionName='Part E' />
            </label>
  ...
  return (
    <BaseSectionComponent
      ...
      api={api}
      renderBreadcrumb={() => (
        <BreadcrumbNav currentPageName='Part E' additionalCrumbs={[]} />
      )}
