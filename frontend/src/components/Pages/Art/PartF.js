...
const PartF = props => {
  ...
  useEffect(() => {
    (async () => {
      setIsLoading(true);
      try {
        const data = await api.getSectionFields(9, 'ART'); // Specify "ART" token type
  ...
  const sectionConfig = {
    sectionNumber: 9,
    sectionTitle: 'Part F: Information on the risks',
    sectionDescription:
        ...
        <HighlightedField key={id} fieldId={id}>
          <div className='mb-10'>
            ...
            >
              ...
              {field.field_name}
              <FlagIcon fieldId={id} section='partF' sectionName='Part F' />
            </label>
  ...
  return (
    <BaseSectionComponent
      ...
      api={api}
      renderBreadcrumb={() => (
        <BreadcrumbNav currentPageName='Part F' additionalCrumbs={[]} />
      )}
