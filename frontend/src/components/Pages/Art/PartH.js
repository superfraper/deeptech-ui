...
const PartH = props => {
  ...
  useEffect(() => {
    (async () => {
      setIsLoading(true);
      try {
        const data = await api.getSectionFields(13, 'ART'); // Specify "ART" token type
  ...
  return (
    <BaseSectionComponent
      ...
      {...sectionConfig}
      api={api}
      tokenType='ART' // Explicitly set token type
      renderBreadcrumb={() => (
        <BreadcrumbNav currentPageName='Part H' additionalCrumbs={[]} />
      )}
