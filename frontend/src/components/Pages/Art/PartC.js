...
const PartC = props => {
  ...
  useEffect(() => {
    (async () => {
      setIsLoading(true);
      try {
        // First try ART token type
        let data;
        try {
          data = await api.getSectionFields(6, 'ART');
        } catch (err) {
          console.warn('Error fetching ART token data:', err.message);
          // If ART fails, fall back to OTH token type
          console.log('Falling back to OTH token type');
          data = await api.getSectionFields(6, 'OTH');
        }
        ...
        setError(
          `Failed to load fields: ${err.message}. The database for ART token type might not be properly configured.`
        );
      } finally {
        setIsLoading(false);
      }
        ...
        <div className='bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 p-4 mb-6'>
          ...
          {fields.length === 0 && (
            <p className='mt-2 text-sm'>
              Note: The ART token type database may not contain the required
              tables. Contact system administrator if this persists.
            </p>
          )}
        </div>
      );
