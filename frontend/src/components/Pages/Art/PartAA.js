...
const PartA = props => {
  ...
  const fetchFields = async () => {
    ...
    try {
      ...
      const data = await api.getSectionFields(4, 'ART'); // Specify "ART" token type
  ...
  const sectionConfig = {
    sectionNumber: 4,
    sectionTitle:
      'Part AA: information on other persons offering to the public or seeking admission to trading of asset-referenced tokens',
    sectionDescription: 'Please fill in these sections',
    nextRoute: '/art/partB', // Route to navigate to next
  };
  ...
  const renderFields = baseProps => {
    ...
    return fields.map(field => {
      ...
      // For non-editable fields, render as read-only
      if (isNonEditable) {
        return (
      ...
      // For all fields, use the default textarea rendering
      return (
        <HighlightedField key={field.field_id} fieldId={field.field_id}>
          <div className='mb-10'>
            <label
