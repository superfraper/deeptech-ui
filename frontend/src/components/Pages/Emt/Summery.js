...
// Create a functional component that uses BaseSectionComponent
const Summery = props => {
  const api = useApi();
  ...
  const sectionConfig = {
    sectionNumber: 2,
    sectionTitle: 'II. (Section 2) Summary',
    sectionDescription: 'These fields are pre populated by DSF',
    nextRoute: '/emt/PartA', // Updated next route to match EMT flow
  };

  // Fetch section fields when component mounts
  useEffect(() => {
    const fetchSectionFields = async () => {
      setIsLoading(true);
      try {
        // Use the API service instead of direct fetch
        const data = await api.getSectionFields(2, 'EMT'); // Specify "EMT" token type
        console.log('Fetched section 2 fields:', data);
        setFields(data.fields || []);
        if (fieldDataError) {
          setError(`Error loading field data: ${fieldDataError}`);
        }
      } catch (error) {
        console.error('Error fetching section 2 fields:', error);
  ...
  // Create a custom renderFields function
  const renderFields = baseProps => {
      ...
      return (
        <div className='bg-red-100 border-l-4 border-red-500 text-red-700 p-4 mb-6'>
          ...
          <p>
            {error ||
              fieldDataError ||
              'No fields found. Please check the database.'}
          </p>
        </div>
      );
    }
    ...
    console.log('Section 2 fields count:', fields.length);
  ...
  const renderBreadcrumb = () => {
    return <BreadcrumbNav currentPageName='Section 2' additionalCrumbs={[]} />;
  };
