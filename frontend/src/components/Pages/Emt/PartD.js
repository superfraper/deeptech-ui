...
const PartD = props => {
  ...
  useEffect(() => {
    (async () => {
      setIsLoading(true);
      try {
        const data = await api.getSectionFields(6, 'EMT'); // Specify "EMT" token type
        ...
        <HighlightedField key={id} fieldId={id}>
          <div className='mb-10'>
            ...
            >
              ...
              {field.field_name}
              <FlagIcon fieldId={id} section='partD' sectionName='Part D' />
            </label>
  ...
  return (
    <BaseSectionComponent
      ...
      api={api}
      renderBreadcrumb={() => (
        <BreadcrumbNav currentPageName='Part D' additionalCrumbs={[]} />
      )}
